export const getServerStats = (conn) => {
            ...
            stream.on('close', () => {
                try {
                    ...
                    for (const line of lines) {
                        ...
                        if (!trimmedLine)
                            continue;
                        if (trimmedLine === 'PROCS:') {
                            parsingProcs = true;
                            parsingConsumers = false;
                            parsingLogins = false;
                            continue;
                        }
                        if (trimmedLine === 'TOP_CONSUMERS:') {
                            parsingConsumers = true;
                            parsingProcs = false;
                            parsingLogins = false;
                            continue;
                        }
                        if (trimmedLine === 'LOGINS:') {
                            parsingLogins = true;
                            parsingProcs = false;
                            parsingConsumers = false;
                            continue;
                        ...
                        if (parsingConsumers && trimmedLine.includes('|')) {
                            ...
                            if (!stats.disk)
                                stats.disk = { device: '', type: '', mount: '', total: '', used: '', free: '', percent: 0, topConsumers: [] };
                        ...
                        if (parsingLogins && trimmedLine.includes('|')) {
                            ...
                            if (user && ip && status && date) {
                                stats.logins.push({ user, ip, status: status, date });
                            }
                            continue;
                        }
                        ...
                        if (key === 'CPU_USAGE')
                            stats.cpu.usage = parseFloat(val) || 0;
                        if (key === 'LOAD_AVG')
                            stats.cpu.loadAvg = val.trim().split(' ').map(Number);
                        if (key === 'CORES')
                            stats.cpu.cores = parseInt(val) || 1;
                        if (key === 'PER_CORE')
                            stats.cpu.perCore = val.trim().split(' ').filter(Boolean).map(v => parseInt(v) || 0);
                        if (key === 'MEM_TOTAL')
                            stats.memory.total = parseInt(val) || 0;
                        if (key === 'MEM_USED')
                            stats.memory.used = parseInt(val) || 0;
                        if (key === 'MEM_FREE')
                            stats.memory.free = parseInt(val) || 0;
                        if (key === 'DISK_TYPE') {
                            if (!stats.disk)
                                stats.disk = { device: '', type: '', mount: '', total: '', used: '', free: '', percent: 0, topConsumers: [] };
