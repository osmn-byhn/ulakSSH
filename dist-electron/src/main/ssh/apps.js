export async function getInstalledApps(conn) {
    const apps = [];
    ...
    try {
        ...
        if (composerOutput) {
            ...
            if (composerData.installed) {
                composerData.installed.forEach((p) => {
                    apps.push({
                        name: p.name,
                        version: p.version,
                        type: 'php',
...
export async function getAppUpdates(conn) {
    const updates = {};
    // 1. APT Upgradable
    try {
...
export async function updateApp(conn, name, type) {
    let cmd = '';
    if (type === 'system') {
...
function categorizeSystemPackage(name, originalType) {
