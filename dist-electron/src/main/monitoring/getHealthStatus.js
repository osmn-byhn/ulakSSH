...
export const getHealthStatus = async (session) => {
    const extractPort = (cmd) => {
    ...
    const cmd = `
        ...
        if command -v forever >/dev/null 2>&1; then
            echo "FOREVER_START"
            forever list --no-color 2>/dev/null
            echo "FOREVER_END"
        fi
        ...
        if command -v docker >/dev/null 2>&1; then
            ...
            if docker compose version >/dev/null 2>&1; then
                docker compose ps --format '{{.Service}}|{{.Status}}|{{.ID}}|{{.Ports}}' 2>/dev/null
    ...
    try {
        ...
        for (const line of lines) {
            ...
            if (trimmed === 'FOREVER_END') {
                ...
                for (const l of foreverLines) {
                    ...
                    if (indexMatch) {
                        ...
                        if (uptimeMatch)
                            currentProc.uptime = uptimeMatch[1];
                    }
                }
                if (currentProc)
                    processes.push(currentProc);
            ...
            if (trimmed === 'DOCKER_COMPOSE_START') {
            ...
            if (trimmed === 'CIRCUS_START') {
                currentSection = 'circus';
                sectionContent = '';
                continue;
            }
            ...
            if (trimmed === 'OXMGR_START') {
                currentSection = 'oxmgr';
                sectionContent = '';
                continue;
            }
            ...
            if (trimmed === 'STRONGPM_START') {
                currentSection = 'strongPm';
                sectionContent = '';
                continue;
            }
